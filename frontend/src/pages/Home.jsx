...
function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const videoPromise = api.get('/videos', {
          params: { query: query || undefined, page: 1, limit: 12 },
        });

        let channelPromise = Promise.resolve({ data: { data: [] } });
        if (query) {
          channelPromise = api.get('/users/search', { params: { query } });
        }
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 md:px-8">
      <h1 className="mb-8 font-heading text-2xl font-bold tracking-tight md:text-3xl">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-xl font-medium">
            {query ? `No results for "${query}"` : 'No videos available'}
          </p>
        </div>
      ) : (
